...
model_number = st.radio("Pick a model number.", model_numbers)
dataset = st.radio("Pick a dataset.", datasets)

if model == 'shave':
    y_true = load('data/y_true.npy').squeeze()
    y_pred = load(f'data/y_pred_{model_number}.npy').squeeze()

if model == 'eff66':
    y_true = load('data/y_true_66.npy').squeeze()
    y_pred = load('data/y_pred_66.npy').squeeze()
...
number = st.number_input(f"Pick a sample number (0-{length})", 0, length)
